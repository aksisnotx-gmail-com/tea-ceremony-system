...
<div>
	<div :style='{"padding":"20px","margin":"64px auto 10px","borderRadius":"0 0 30px 30px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"15px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discusschadaozhishiku"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"550px","width":"45%","padding":"0 20px","position":"relative","background":"#fff","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"0","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discusschadaozhishiku','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFDFA680","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discusschadaozhishiku','删除')" @click="delClick">删除</el-button>
					<!-- hasChat $hasChat -->
				</div>
			</div>
		</div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        tablename: 'discusschadaozhishiku',
        baseUrl: '',
        breadcrumbItem: [
          {
            name: '茶道知识库评论表'
    ...
    methods: {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscusschadaozhishiku/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscusschadaozhishiku/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscusschadaozhishiku/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discusschadaozhishiku/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discusschadaozhishikuAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此茶道知识库评论表？')
			  .then(_ => {
			    this.$http.post('discusschadaozhishiku/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
