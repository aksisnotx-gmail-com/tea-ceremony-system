...
<div>
	<div :style='{"padding":"20px","margin":"64px auto 10px","borderRadius":"0 0 30px 30px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"15px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/chadaozhishiku"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 10%","margin":"10px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#F6F6F6","display":"flex","width":"100%","position":"relative"}'>
		<div class="attr" :style='{"minHeight":"550px","width":"45%","padding":"0 20px","position":"relative","background":"#fff","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"0","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>知识分类</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.zhishifenlei}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#9e9e9e","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"30px","height":"30px"}'>发布时间</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"30px","color":"#000","flex":"1","height":"30px"}'>{{detail.fabushijian}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"linear-gradient(90deg, #5FE3EB 0%, #FFDFA6 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('chadaozhishiku','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFDFA680","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('chadaozhishiku','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
		...
		<video :style='{"border":"0","width":"100%","margin":"10px auto","outline":"none","borderRadius":"4px","display":"block"}' :src="baseUrl +  detail.shipin" controls="controls">
			您的浏览器不支持视频播放
		</video>
  ...
  export default {
    ...
    methods: {
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('chadaozhishiku/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'chadaozhishiku', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discusschadaozhishiku/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discusschadaozhishiku/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusschadaozhishiku/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/chadaozhishikuAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此茶道知识库？')
			  .then(_ => {
			    this.$http.post('chadaozhishiku/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
