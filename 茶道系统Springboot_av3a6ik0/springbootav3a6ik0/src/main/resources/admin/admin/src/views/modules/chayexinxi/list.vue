<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">茶叶名称</label>
						<el-input v-model="searchForm.chayemingcheng" placeholder="茶叶名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="茶叶分类" prop="chayefenlei">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">茶叶分类</label>
						<el-select clearable v-model="searchForm.chayefenlei" placeholder="请选择茶叶分类" >
							<el-option v-for="(item,index) in chayefenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">产地</label>
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('chayexinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('chayexinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0 0 2px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","borderColor":"rgba(252,234,238,1)","margin":"0 0 20px","borderRadius":"3px","background":"rgba(255,255,255,1)","borderWidth":"0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('chayexinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan12" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('chayexinxi','到货入库')" @click="daohuorukuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								到货入库
							</el-button>
							<el-button class="edit" v-if=" isAuth('chayexinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								更改
							</el-button>

							<el-button class="view" v-if="isAuth('chayexinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan12" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"inherit","height":"auto"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('chayexinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<daohuoruku-cross-add-or-update v-if="daohuorukuCrossAddOrUpdateFlag" :parent="this" ref="daohuorukuCrossaddOrUpdate"></daohuoruku-cross-add-or-update>
...
import daohuorukuCrossAddOrUpdate from "../daohuoruku/add-or-update";
	export default {
		data() {
			return {
				chayefenleiOptions: [],
				...
				daohuorukuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			daohuorukuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.daohuorukuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','chayexinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.daohuorukuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.daohuorukuCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          this.$http({
            url: `option/chayefenlei/chayefenlei`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.chayefenleiOptions = data.data;
            } else {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.chayemingcheng!='' && this.searchForm.chayemingcheng!=undefined){
            params['chayemingcheng'] = '%' + this.searchForm.chayemingcheng + '%'
          }
           if(this.searchForm.chayefenlei!='' && this.searchForm.chayefenlei!=undefined){
            params['chayefenlei'] = this.searchForm.chayefenlei
          }
	...
	this.$router.push({path:'/discusschayexinxi',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	chayexinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'chayexinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
